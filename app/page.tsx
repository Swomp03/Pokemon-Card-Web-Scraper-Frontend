...
import AllSetsButton from "../public/assets/AllSetsButton.png";
...
type MyTimestamp = number | string | Date;
...
export default function Home() {
  ...
  const setList = useAsyncList<setButton>({
    async load({ signal }) {
      ...
      for (let i = 0; i < data.length; i++) {
        selectedSets.push(data[i].setCode);
        console.log("Selected Sets:", selectedSets);
      }
  ...
  useEffect(() => {
    const loadImages = async () => {
      ...
      for (const item of setList.items) {
        ...
        try {
          const imageModule = await import(`../public/assets/${imageName}.png`);
